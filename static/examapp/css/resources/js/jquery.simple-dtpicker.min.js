...
$picker.parent().css('left',$input.offset().left+'px');$picker.parent().css('z-index',100000)}};var PickerObjects=[];var InputObjects=[];var ActivePickerId=-1;var getParentPickerObject=function(obj){return $(obj).closest('.datepicker')};var getPickersInputObject=function($obj){var $picker=getParentPickerObject($obj);if($picker.data("inputObjectId")!=null){return $(InputObjects[$picker.data("inputObjectId")])}
...
var date=getPickedDate($picker);var targetMonth_lastDay=new Date(date.getFullYear(),date.getMonth(),0).getDate();if(targetMonth_lastDay<date.getDate()){date.setDate(targetMonth_lastDay)}
draw($picker,{"isAnim":!0,"isOutputToInputObject":!0},date.getFullYear(),date.getMonth()-1,date.getDate(),date.getHours(),date.getMinutes());var todayDate=new Date();var isCurrentYear=todayDate.getFullYear()==date.getFullYear();var isCurrentMonth=isCurrentYear&&todayDate.getMonth()==date.getMonth();if(!isCurrentMonth||!$picker.data("futureOnly")){if(targetMonth_lastDay<date.getDate()){date.setDate(targetMonth_lastDay)}
...
draw($picker,{"isAnim":!0,"isOutputToInputObject":!0},date.getFullYear(),date.getMonth()+1,date.getDate(),date.getHours(),date.getMinutes())};var getLastDate=function(year,month){var date=new Date(year,month+1,0);return date.getDate()};var getDateFormat=function(format,locale,is_date_only){if(format=="default"){format=translate(locale,'format');if(is_date_only){format=format.substring(0,format.search(' '))}}
...
return year};var parseDate=function(str,opt_date_format){var re,m,date;if(opt_date_format!=null){var df=opt_date_format.replace(/(-|\/)/g,'[-\/]').replace(/YYYY/gi,'(\\d{2,4})').replace(/(YY|MM|DD|hh|mm)/g,'(\\d{1,2})').replace(/(M|D|h|m)/g,'(\\d{1,2})');re=new RegExp(df);m=re.exec(str);if(m!=null){var formats=[];var format_buf='';var format_before_c='';var df_=opt_date_format;while(df_!=null&&0<df_.length){var format_c=df_.substring(0,1);df_=df_.substring(1,df_.length);if(format_before_c!=format_c){if(/(YYYY|YY|MM|DD|mm|dd|M|D|h|m)/.test(format_buf)){formats.push(format_buf);format_buf=''}else{format_buf=''}}
format_buf+=format_c;format_before_c=format_c}
if(format_buf!==''&&/(YYYY|YY|MM|DD|mm|dd|M|D|h|m)/.test(format_buf)){formats.push(format_buf)}
var year,month,day,hour,min;var is_successful=!1;for(var i=0;i<formats.length;i++){if(m.length<i){break}
var f=formats[i];var d=m[i+1];if(f=='YYYY'){year=normalizeYear(d);is_successful=!0}else if(f=='YY'){year=parseInt(d)+2000;is_successful=!0}else if(f=='MM'||f=='M'){month=parseInt(d)-1;is_successful=!0}else if(f=='DD'||f=='D'){day=d;is_successful=!0}else if(f=='hh'||f=='h'){hour=d;is_successful=!0}else if(f=='mm'||f=='m'){min=d;is_successful=!0}}
...
var y=date.getFullYear();var m=date.getMonth()+1;var d=date.getDate();var hou=date.getHours();var min=date.getMinutes();date_format=date_format.replace(/YYYY/gi,y).replace(/YY/g,y-2000).replace(/MM/g,zpadding(m)).replace(/M/g,m).replace(/DD/g,zpadding(d)).replace(/D/g,d).replace(/hh/g,zpadding(hou)).replace(/h/g,hou).replace(/mm/g,zpadding(min)).replace(/m/g,min);return date_format};var outputToInputObject=function($picker){var $inp=getPickersInputObject($picker);if($inp==null){return}
...
var isTodayButton=$picker.data("todayButton");var isScroll=option.isAnim;if($picker.data("timelistScroll")===!1){isScroll=!1}
var isAnim=option.isAnim;if($picker.data("animation")===!1){isAnim=!1}
var isFutureOnly=$picker.data("futureOnly");var minDate=$picker.data("minDate");var maxDate=$picker.data("maxDate");var isOutputToInputObject=option.isOutputToInputObject;var minuteInterval=$picker.data("minuteInterval");var firstDayOfWeek=$picker.data("firstDayOfWeek");var allowWdays=$picker.data("allowWdays");if(allowWdays==null||isObj('Array',allowWdays)===!1||allowWdays.length<=0){allowWdays=null}
var minTime=$picker.data("minTime");var maxTime=$picker.data("maxTime");var todayDate=new Date();if(isFutureOnly){if(date.getTime()<todayDate.getTime()){date.setTime(todayDate.getTime())}}
...
$($picker).data("pickedDate",date);if(isAnim===!0){if(changePoint=="calendar"){$calendar.stop().queue([]);$calendar.fadeTo("fast",0.8)}else if(changePoint=="timelist"){$timelist.stop().queue([]);$timelist.fadeTo("fast",0.8)}}
...
var realDayObj=new Date(date.getTime());realDayObj.setHours(0);realDayObj.setMinutes(0);realDayObj.setSeconds(0);for(var zz=0;i<cellNum;i++){var realDay=i+1-firstWday;var isPast=isPastMonth||(isCurrentMonth&&realDay<todayDate.getDate());if(i%7===0){$tr=$('<tr>');$table.append($tr)}
...
var realDayObjMN=new Date(realDayObj.getTime());realDayObjMN.setHours(23);realDayObjMN.setMinutes(59);realDayObjMN.setSeconds(59);if(((minDate!=null)&&(minDate>realDayObjMN.getTime()))||((maxDate!=null)&&(maxDate<realDayObj.getTime()))){$td.addClass('out_of_range')}else if(isFutureOnly&&isPast){$td.addClass('day_in_past')}else{$td.click(function(){if($(this).hasClass('hover')){$(this).removeClass('hover')}
$(this).addClass('active');var $picker=getParentPickerObject($(this));var targetDate=new Date($(this).data("dateStr"));var selectedDate=getPickedDate($picker);draw($picker,{"isAnim":!1,"isOutputToInputObject":!0},targetDate.getFullYear(),targetDate.getMonth(),targetDate.getDate(),selectedDate.getHours(),selectedDate.getMinutes());if($picker.data("dateOnly")===!0&&$picker.data("isInline")===!1&&$picker.data("closeOnSelected")){ActivePickerId=-1;$picker.hide()}});$td.hover(function(){if(!$(this).hasClass('active')){$(this).addClass('hover')}},function(){if($(this).hasClass('hover')){$(this).removeClass('hover')}})}}
...
realDayObj=new Date(date.getTime());$timelist.css("height",$calendar.innerHeight()-10+'px');var hour_=minTime[0];var min_=minTime[1];while(hour_*100+min_<maxTime[0]*100+maxTime[1]){var $o=$('<div>');var is_past_time=hour_<todayDate.getHours()||(hour_==todayDate.getHours()&&min_<todayDate.getMinutes());var is_past=isCurrentDay&&is_past_time;$o.addClass('timelist_item');$o.text(zpadding(hour_)+":"+zpadding(min_));$o.data("hour",hour_);$o.data("min",min_);$timelist.append($o);realDayObj.setHours(hour_);realDayObj.setMinutes(min_);if(((minDate!=null)&&(minDate>realDayObj.getTime()))||((maxDate!=null)&&(maxDate<realDayObj.getTime()))){$o.addClass('out_of_range')}else if(isFutureOnly&&is_past){$o.addClass('time_in_past')}else{$o.click(function(){if($(this).hasClass('hover')){$(this).removeClass('hover')}
...
$.event.special.destroyed={remove:function(o){if(o.handler){o.handler.apply(this,arguments)}}};$(function(){$('body').click(function(){for(var i=0;i<PickerObjects.length;i++){var $picker=$(PickerObjects[i]);if(ActivePickerId!=i){if($picker.data("inputObjectId")!=null&&$picker.data("isInline")===!1&&$picker.css('display')!='none'){var $input=InputObjects[$picker.data("inputObjectId")];var handler=new PickerHandler($picker,$input);handler.hide();var func=$picker.data('onHide');if(func!=null){console.log("dtpicker- Call the onHide handler");func(handler)}}}}})})})(jQuery)